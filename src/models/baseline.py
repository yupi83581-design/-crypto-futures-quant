...
class MajorityClassBaseline:
    ...
    def predict(self, count: int | Sequence[float]) -> list[int]:
        """Predict the fitted majority class for a count or feature sequence."""
        self._require_fitted()
        prediction_count = _prediction_count(count)
        return [self._majority_class] * prediction_count  # type: ignore[list-item]

    def predict_proba(self, features: int | Sequence[float]) -> list[float]:
        """Return class-1 probabilities for a count or feature sequence."""
        self._require_fitted()
        prediction_count = _prediction_count(features)
        probability = float(self._majority_class)
        return [probability] * prediction_count
...
class LogisticRegressionBaseline:
    ...
    def fit(self, features: Sequence[float], labels: Sequence[int]) -> "LogisticRegressionBaseline":
        ...
        if len(x) != len(y):
            raise ValueError("features and labels must contain the same number of observations")
        if not x:
            raise ValueError("features and labels must not be empty")
        feature_mean = sum(x) / len(x)
        variance = sum((value - feature_mean) ** 2 for value in x) / len(x)
        feature_scale = math.sqrt(variance) or 1.0
        standardized = [(value - feature_mean) / feature_scale for value in x]
        weight = 0.0
        bias = 0.0
        n = float(len(standardized))
        for _ in range(self.iterations):
            probabilities = [_sigmoid(weight * value + bias) for value in standardized]
            weight_gradient = sum(
                (probability - target) * value
                for probability, target, value in zip(probabilities, y, standardized)
            ) / n
            bias_gradient = sum(probability - target for probability, target in zip(probabilities, y)) / n
    ...
    def predict_proba(self, features: Sequence[float]) -> list[float]:
        ...
        self._require_fitted()
        x = _validate_features(features)
        return [self._predict_probability(value) for value in x]

    def predict(self, features: Sequence[float], threshold: float = 0.5) -> list[int]:
    ...
    def _predict_probability(self, feature: float) -> float:
        ...
        standardized = (feature - self._feature_mean) / self._feature_scale
        return _sigmoid(self._weight * standardized + self._bias)

    def _require_fitted(self) -> None:
        if self._weight is None or self._bias is None or self._feature_mean is None or self._feature_scale is None:
            raise RuntimeError("model must be fitted before prediction")


def _prediction_count(value: int | Sequence[float]) -> int:
    """Normalize legacy counts and production feature sequences."""
    if isinstance(value, int) and not isinstance(value, bool):
        if value < 0:
            raise ValueError("count must be a non-negative integer")
        return value
    if isinstance(value, (str, bytes)):
        raise ValueError("prediction input must be a count or feature sequence")
    try:
        return len(value)
    except TypeError as exc:
        raise ValueError("prediction input must be a count or feature sequence") from exc
...
def _validate_features(features: Sequence[float]) -> list[float]:
    ...
    for index, value in enumerate(features):
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError(f"feature at index {index} must be numeric, got {value!r}")
        numeric = float(value)
        if not math.isfinite(numeric):
            raise ValueError(f"feature at index {index} must be finite, got {value!r}")
...
def _validate_labels(labels: Sequence[int]) -> list[int]:
    ...
    for index, label in enumerate(labels):
        if isinstance(label, bool) or not isinstance(label, int):
            raise ValueError(f"label at index {index} must be an integer 0 or 1")
        if label not in (0, 1):
            raise ValueError(f"label at index {index} must be 0 or 1, got {label!r}")
