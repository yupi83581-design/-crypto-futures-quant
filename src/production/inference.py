...
class ProductionInferenceEngine:
    ...
    def run(self, data: InferenceInput) -> InferenceResult:
        ...
        if not records:
            raise ValueError("market_data_adapter returned no market data")

        features = self._feature_builder(records)

        if isinstance(features, (str, bytes)):
            raise TypeError("feature_builder must return a sequence")
        ...
        if len(features) != len(records):
            raise ValueError(
                "feature sequence length must match market-data observation "
                "count before probability count can be validated"
            )
        ...
        for value in features:
            if value is None:
                continue
            usable_features.append(self._validate_feature(value))

        if not usable_features:
            raise ValueError("feature_builder produced no usable features")

        probabilities = self._model.predict_proba(usable_features)

        if isinstance(probabilities, (str, bytes)):
            raise TypeError("model.predict_proba must return a sequence")
    ...
    def _validate_input(data: InferenceInput) -> None:
        if not isinstance(data, InferenceInput):
            raise TypeError("data must be an InferenceInput")
        ...
        ProductionInferenceEngine._validate_time(data.start_time, "start_time")
        ProductionInferenceEngine._validate_time(data.end_time, "end_time")

    @staticmethod
    def _validate_time(value: str | datetime, field_name: str) -> None:
    ...
    @staticmethod
    def _validate_feature(value: Any) -> float:
        if isinstance(value, bool):
            raise TypeError("feature must be numeric")
