...
class TestMajorityClassBaseline:
    ...
    def test_feature_sequence_prediction_contract(self) -> None:
        model = MajorityClassBaseline()
        model.fit([0, 0, 1])
        features = [10.0, 20.0, 30.0]
        assert model.predict(features) == [0, 0, 0]
        assert model.predict_proba(features) == [0.0, 0.0, 0.0]

    def test_fit_returns_self(self) -> None:
        model = MajorityClassBaseline()
        assert model.fit([0, 1, 1]) is model

    def test_predict_before_fit_fails(self) -> None:
        with pytest.raises(RuntimeError, match="must be fitted"):
            MajorityClassBaseline().predict(1)
    ...
    def test_invalid_labels_fail(self) -> None:
        with pytest.raises(ValueError, match="0 or 1"):
            MajorityClassBaseline().fit([0, 2])

    def test_boolean_labels_fail(self) -> None:
        with pytest.raises(ValueError, match="integer"):
            MajorityClassBaseline().fit([0, True])
...
class TestLogisticRegressionBaseline:
    def test_fit_returns_self(self) -> None:
        ...
        result = model.fit([-2.0, -1.0, 1.0, 2.0], [0, 0, 1, 1])
        assert result is model

    def test_learns_positive_relationship(self) -> None:
        model = LogisticRegressionBaseline(learning_rate=0.1, iterations=3000, regularization=0.01)
        model.fit([-3.0, -2.0, -1.0, 1.0, 2.0, 3.0], [0, 0, 0, 1, 1, 1])
    ...
    def test_predict_uses_default_threshold(self) -> None:
        model = LogisticRegressionBaseline(learning_rate=0.1, iterations=3000)
        model.fit([-3.0, -2.0, -1.0, 1.0, 2.0, 3.0], [0, 0, 0, 1, 1, 1])
        assert model.predict([-2.0, 2.0]) == [0, 1]
    ...
    def test_probabilities_are_between_zero_and_one(self) -> None:
        model = LogisticRegressionBaseline(learning_rate=0.1, iterations=3000)
        model.fit([-3.0, -2.0, -1.0, 1.0, 2.0, 3.0], [0, 0, 0, 1, 1, 1])
        probabilities = model.predict_proba([-100.0, -1.0, 0.0, 1.0, 100.0])
    ...
    def test_predict_before_fit_fails(self) -> None:
        with pytest.raises(RuntimeError, match="must be fitted"):
            LogisticRegressionBaseline().predict([1.0])

    def test_predict_proba_before_fit_fails(self) -> None:
        with pytest.raises(RuntimeError, match="must be fitted"):
            LogisticRegressionBaseline().predict_proba([1.0])

    def test_feature_label_length_mismatch_fails(self) -> None:
        with pytest.raises(ValueError, match="same number of observations"):
            LogisticRegressionBaseline().fit([1.0, 2.0], [0])

    def test_empty_training_data_fails(self) -> None:
        with pytest.raises(ValueError, match="must not be empty"):
            LogisticRegressionBaseline().fit([], [])

    def test_invalid_feature_fails(self) -> None:
        with pytest.raises(ValueError, match="must be numeric"):
            LogisticRegressionBaseline().fit([1.0, "bad"], [0, 1])  # type: ignore[list-item]

    def test_boolean_feature_fails(self) -> None:
        with pytest.raises(ValueError, match="must be numeric"):
            LogisticRegressionBaseline().fit([1.0, True], [0, 1])

    def test_non_finite_feature_fails(self) -> None:
        with pytest.raises(ValueError, match="must be finite"):
            LogisticRegressionBaseline().fit([1.0, float("nan")], [0, 1])

    def test_invalid_label_fails(self) -> None:
        with pytest.raises(ValueError, match="0 or 1"):
            LogisticRegressionBaseline().fit([1.0, 2.0], [0, 2])

    def test_boolean_label_fails(self) -> None:
        with pytest.raises(ValueError, match="integer"):
            LogisticRegressionBaseline().fit([1.0, 2.0], [0, True])

    def test_constant_features_are_supported(self) -> None:
        model = LogisticRegressionBaseline()
        model.fit([5.0, 5.0, 5.0, 5.0], [0, 0, 1, 1])
