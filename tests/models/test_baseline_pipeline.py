...
def make_future_records(
    ...
    """Build chronologically ordered records after an existing dataset."""
    return [
        {
            "event_time": f"2026-01-01T00:{start_minute + index:02d}:00Z",
            "close": close,
        }
        for index, close in enumerate(closes)
    ]
...
def test_pipeline_fit_returns_self() -> None:
    records = make_records(
        [
            100, 101, 102, 101, 100, 101, 102, 103, 102,
            104, 105, 104, 106, 107, 108, 109, 108, 110, 111,
        ]
    )
...
def test_pipeline_evaluate_returns_expected_result_contract() -> None:
    training = make_records(
        [
            100, 101, 102, 103, 102, 104, 105, 104, 106, 107,
    ...
    evaluation = make_records(
        [
            200, 201, 202, 201, 203, 204, 203, 205, 206, 207,
            206, 208, 209, 210, 211, 210, 212, 213, 214, 215,
        ]
    )
...
def test_pipeline_probability_predictions_use_configured_threshold() -> None:
    training = make_records(
        [
            100, 101, 102, 103, 104, 103, 105, 106, 107, 106,
            108, 109, 110, 111, 112, 113, 112, 114, 115, 116,
        ]
    )

    evaluation = make_records(
        [
            200, 201, 202, 203, 202, 204, 205, 206, 205, 207,
            208, 209, 210, 211, 210, 212, 213, 214, 215,
        ]
    )

    pipeline = BaselineRSIPipeline(threshold=0.7)
...
def test_pipeline_rejects_missing_close() -> None:
    ...
    with pytest.raises(
        ...
        BaselineRSIPipeline().fit(records)
...
def test_pipeline_output_is_deterministic() -> None:
    ...
    evaluation = make_records(
        [
            200, 201, 200, 202, 203, 204, 203, 205, 206, 207,
            206, 208, 209, 210, 211, 210, 212, 213, 214, 215,
        ]
    )
...
def test_pipeline_has_no_future_label_for_final_horizon_records() -> None:
    ...
    pipeline = BaselineRSIPipeline(label_horizon=3)
...
def test_pipeline_does_not_use_future_records_for_rsi_features() -> None:
    ...
    extended = prefix + future_records
    ...
    assert (
        prefix_dataset.features
        == extended_dataset.features[:prefix_feature_count]
    )
...
def test_pipeline_probability_values_are_finite() -> None:
    training = make_records(
        [
            100, 101, 102, 103, 102, 104, 105, 106, 105, 107,
            108, 109, 110, 109, 111, 112, 113, 114, 113, 115,
        ]
    )

    evaluation = make_records(
        [
            200, 201, 202, 201, 203, 204, 205, 204, 206, 207,
            208, 207, 209, 210, 211, 212, 211, 213, 214, 215,
        ]
    )
