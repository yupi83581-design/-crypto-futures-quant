...
def base(**kwargs):
    params={"equity":100_000,"entry_price":100,"stop_price":98,"net_expected_value":.02}; params.update(kwargs); return assess_risk(**params)
...
def test_zero_and_negative_ev_are_rejected():
    assert base(net_expected_value=0).approved is False; assert base(net_expected_value=-.01).approved is False

def test_invalid_stop_is_rejected():
    assert base(stop_price=100).approved is False; assert base(stop_price=101).approved is False
...
def test_drawdown_limit_is_enforced():
    r=base(config=RiskConfig(max_drawdown_fraction=.20),current_drawdown_fraction=.20); assert r.approved is False; assert r.reason=="max drawdown limit reached"

def test_drawdown_below_limit_is_allowed():
    assert base(config=RiskConfig(max_drawdown_fraction=.20),current_drawdown_fraction=.199).approved is True
