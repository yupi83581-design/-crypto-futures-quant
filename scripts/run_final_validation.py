...
def main() -> int:
    ...
    if phase10 and phase10.get("status") == "COMPLETE":
        ...
        evidence["dsr"] = GateStatus.PASS.value if (0.0 < dsr.get("deflated_sharpe_probability", -1) <= 1.0 and dsr.get("observed_sharpe", 0.0) > 0.0 and math.isfinite(float(dsr.get("trial_sharpe_variance", float("nan")))) and dsr.get("trial_count", 0) == len(phase10.get("method", {}).get("strategy_universe", [])) and len(dsr.get("trial_universe", [])) == dsr.get("trial_count", 0)) else (GateStatus.FAIL.value if dsr.get("observed_sharpe") is not None else GateStatus.INSUFFICIENT_EVIDENCE.value)
        evidence["pbo_cscv"] = GateStatus.PASS.value if (pbo.get("path_count", 0) > 0 and len(pbo.get("omega_values", [])) == pbo.get("path_count", 0) and len(pbo.get("logit_values", [])) == pbo.get("path_count", 0) and len(pbo.get("selected_strategy_indices", [])) == pbo.get("path_count", 0) and pbo.get("strategy_count", 0) == dsr.get("trial_count", 0) and pbo.get("observations", 0) == pbo.get("input_observations_before_trim", pbo.get("observations", 0)) - pbo.get("trimmed_observations", 0) and 0 <= pbo.get("trimmed_observations", -1) < pbo.get("block_count", 0) and pbo.get("trim_policy") == "drop trailing observations so aligned time-series length is divisible by block_count") else GateStatus.FAIL.value
